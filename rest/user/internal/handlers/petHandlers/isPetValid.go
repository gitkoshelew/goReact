...
func PetValidationHandle(s *store.Store) httprouter.Handle {
	return func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		...
		if err != nil {
			...
			json.NewEncoder(w).Encode(apperror.NewAppError(fmt.Sprintf("Bad request. Err msg:%v. Requests body: %v", err, ps.ByName("petId")),
		...
		if err != nil {
			...
			json.NewEncoder(w).Encode(apperror.NewAppError("err occured while requesting DB", fmt.Sprintf("%d", http.StatusInternalServerError),
				fmt.Sprintf("err occured while requesting DB. Err msg: %v", err)))
			return
		}
