...
var permission_delete model.Permission = model.Permission{Name: model.DeletePet}
...
func DeletePets(s *store.Store) httprouter.Handle {
	return func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		...
		err = s.Pet().Delete(id)
		if err != nil {
			http.Error(w, fmt.Sprintf("Error occured while deleting all pets. Err msg:%v. ", err), http.StatusInternalServerError)
			return
		}
