...
func GetEmployeeByID(s *store.Store) httprouter.Handle {
	return func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		...
		if err != nil {
			w.WriteHeader(http.StatusBadRequest)
			json.NewEncoder(w).Encode(apperror.NewAppError("Error occured while getting employee by id.", fmt.Sprintf("%d", http.StatusBadRequest), fmt.Sprintf("Error occured while getting employee by id. Err msg:%v.", err)))
			return
		}
