...
func (s *Server) configureRouter() {
	...
	s.router.Handle("POST", "/api/forgotpassword", authentication.ForgotPassword(store.New(s.config), s.Mail))
	s.router.Handle("GET", "/api/emailrestore/:token", authentication.СhekingLinkForRestorePassword(store.New(s.config)))
	s.router.Handle("POST", "/api/passwordchange", authentication.ChangePassword(store.New(s.config)))
