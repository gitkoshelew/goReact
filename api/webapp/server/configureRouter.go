...
func (s *Server) configureRouter() {
	...
	s.router.Handler("GET", "/private/login", middleware.IsLoggedIn(authentication.LoginHandle(store.New(s.config))))

	s.router.Handle("POST", "/api/registration", authentication.RegistrationHandle(store.New(s.config)))
	s.router.Handler("POST", "/api/login", authentication.LoginHandle(store.New(s.config)))
